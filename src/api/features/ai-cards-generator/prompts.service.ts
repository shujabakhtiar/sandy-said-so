export class PromptsService {
  static getPromptForMode(modeName: string, deck: any, variation: { instruction: string; type: string }) {
    switch (modeName) {
  ...
  private static buildConfessionPrompt(deck: any, variation: { instruction: string; type?: string }) {
...
Example: ["Sandy says: [Name], point to the person you'd least trust with your phone.", "Confess your cringiest high school memory or take 3 sips."]`;
  ...
  private static buildProvocationPrompt(deck: any, variation: { instruction: string; type?: string }) {
    const context = this.getCleanContext(deck);
...
Example: ["Sandy says: Categories! Types of shots. [Name] starts.", "Thumb Master: When you put your thumb on the table, everyone follows. Last person drinks."]`;
  ...
  private static buildDimmedLightsPrompt(deck: any, variation: { instruction: string; type: string }) {
    const context = this.getCleanContext(deck);
    const isPhase0 = variation.type === "PHASE_0";
...
IMPORTANT STRUCTURE:
- ${isPhase0 
    ? "Generate exactly ONE short narrative paragraph for this phase. This card is SHARED." 
    : "Generate exactly TWO PRIVATE CARDS: His Card and Her Card. One for Him, one for Her."}
...
${this.getTechnicalConstraints(isPhase0 ? 1 : 2, true)}

Context Summary: ${context.summary}`;
  }

  private static buildStandardPrompt(deck: any, variation: { instruction: string; type?: string }) {
    const context = this.getCleanContext(deck);
...
Context: ${context.summary}`;
  ...
  private static getTechnicalConstraints(cardCount: number = 10, isDimmedLights: boolean = false) {
    const formatSpec = isDimmedLights 
      ? `OUTPUT: Valid JSON array of OBJECTS. Each object must have: 
   - "text": the card content string
   - "target": "Him", "Her", or "Both" (Both only for PHASE 0)`
...
3. STRUCTURE: Flat array of ${isDimmedLights ? "objects" : "strings"}.
4. VALIDATION: Keys and values must use double quotes.
5. LIMITS: Exactly ${cardCount} ${isDimmedLights ? "objects" : "strings"}. ${cardCount === 1 ? "Maximum 80 words." : "Maximum 40 words per string."}`;
  }
}
