...
export const gameDecksResource = {
  ...
  getById: <T = any>(id: string | number, params?: PaginationParams & { isDraft?: boolean }) => {
    const searchParams = new URLSearchParams();
    if (params?.page) searchParams.append("page", String(params.page));
    if (params?.limit) searchParams.append("limit", String(params.limit));
    if (params?.isDraft !== undefined) searchParams.append("isDraft", String(params.isDraft));
    
    const queryString = searchParams.toString();
    const url = queryString ? `/api/game-decks/${id}?${queryString}` : `/api/game-decks/${id}`;
